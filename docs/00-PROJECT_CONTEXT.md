...
| Phase 24: Retroactive Enrichment & Sensitivity Scanning | ✅ Complete | `scanSensitivity()` regex in `brain-engine.ts`. `sensitivity_tier` column in `memories` (migration `023`). Sensitivity gates in `ingest-thought` and `capture_memory`. Local backfill CLI at `.agents/skills/retroactive-enrichment/backfill.ts`. |
